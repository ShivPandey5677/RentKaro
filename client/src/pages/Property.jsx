...
const Property = () => {
  ...
  const handleSearch = async () => {
    try {
      setIsLoading(true);
      const response = await makeRequest.get('/property', { params: property });
  ...
  />
  <label htmlFor="ptype" className="text-black">Flat</label>
  ...
  />
  <label htmlFor="ptype" className=" text-black">PG</label>
  <input
    type="radio"
    id="propertytype"
    name="type"
    ...
    onChange={(e) => handleInputChange("type", e.target.value)}
                       className="border-none border-b-2 border-blue-100 p-2 rounded-md mx-2"
  />
  <label htmlFor="ptype" className="text-black">Full House</label>
